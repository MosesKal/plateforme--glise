...
export type AudioProcessingStatus =
  ...
  | "READY"
  | "FAILED";
export type AdminAudioMediaFilter = AudioProcessingStatus | "MISSING";
...
export const adminTeachingsApi = {
  ...
  updateTheme: (id: string, payload: Partial<ThemePayload>) =>
    api
      .patch<AdminTheme>(`/teachings/themes/${id}`, payload)
      .then((r) => r.data),
  ...
  updateSpeaker: (id: string, payload: Partial<SpeakerPayload>) =>
    api
      .patch<Speaker>(`/teachings/speakers/${id}`, payload)
      .then((r) => r.data),
  ...
  createAudio: (payload: AudioTeachingPayload) =>
    api
      .post<AdminAudioTeaching>("/teachings/audio", payload)
      .then((r) => r.data),
  ...
  discardAudioUpload: (uploadId: string) =>
    api.delete(`/teachings/audio/uploads/${uploadId}`).then((r) => r.data),

  findAudioUpload: (uploadId: string) =>
    api
      .get<AudioUploadResult>(`/teachings/audio/uploads/${uploadId}`)
      .then((r) => r.data),

  retryAudioProcessing: (id: string) =>
    api.post(`/teachings/audio/${id}/retry-processing`).then((r) => r.data),
  ...
  syncVideos: () =>
    api
      .post<VideoSyncResult>("/teachings/videos/sync", undefined, {
        timeout: 0,
      })
      .then((r) => r.data),
  ...
  videoSyncStatus: () =>
    api
      .get<VideoSyncStatus>("/teachings/videos/sync/status")
      .then((r) => r.data),
  ...
  uploadAudio: (
    ...
    return api
      .post<AudioUploadResult>("/teachings/audio/upload", form, {
        timeout: 0,
        headers: { "X-Audio-Upload-Id": uploadId },
        onUploadProgress: (e) => {
