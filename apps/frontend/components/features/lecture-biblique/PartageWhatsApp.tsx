...
export function PartageWhatsApp({ reading }: PartageWhatsAppProps) {
  ...
  const todayChapters = reading.todayData?.chapters ?? []

  const buildMessage = () => {
    const chapterLines = todayChapters
      .map((ch) => `${bookName(ch.bookId, loc)} ${ch.chapter}`)
      .join("\n")

    return (
      `${t("lectureBibliquePage.share_msg_header")}\n\n` +
      `${chapterLines}\n\n` +
  ...
  if (todayChapters.length === 0) return null
        ...
        <motion.div variants={stagger} {...inView()} className="space-y-8">
          ...
          <motion.div variants={fadeUp} className="text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 lg:text-3xl">
              {t("lectureBibliquePage.share_title")}
            </h2>
          </motion.div>
          ...
          >
            <div className="mb-4 rounded-xl bg-gray-50 p-4 font-mono text-sm leading-relaxed text-gray-700 whitespace-pre-line">
              {buildMessage()}
            </div>
            <button
              onClick={shareOnWhatsApp}
              className="flex w-full items-center justify-center gap-3 rounded-full bg-[#25D366] py-4 text-base font-semibold text-white shadow-md transition-all hover:-translate-y-0.5 hover:bg-[#1DA851] hover:shadow-lg"
            >
              {/* WhatsApp icon */}
