...
export function AProposContent() {
  ...
  const whatsappUrl = CHURCH_INFO.socials.whatsappChannel ?? "#"
      ...
      <section id="contact" className="bg-cecj-green/5 py-20 md:py-28">
        <div className="mx-auto max-w-4xl px-4 lg:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <motion.div {...inView()} variants={stagger} className="space-y-4">
              {[
                { icon: ICONS.phone, label: t("apropos.contact_phone_label"),   value: CHURCH_INFO.contact.phone },
                { icon: ICONS.pin,   label: t("apropos.contact_address_label"), value: CHURCH_INFO.location.fullAddress },
                { icon: ICONS.clock, label: t("apropos.contact_hours_label"),   value: t("apropos.contact_hours_value") },
              ].map(({ icon, label, value }) => (
                <motion.div key={label} variants={fadeUp} className="flex items-start gap-4 rounded-2xl border border-white bg-white p-5 shadow-sm">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cecj-green/10">
                    <Icon d={icon} className="h-5 w-5 text-cecj-green" />
                  </span>
                  <div>
            ...
            {/* Actions */}
            <motion.div {...inView()} variants={stagger} className="flex flex-col gap-4">
              <motion.a
                variants={fadeUp}
                href={`tel:${phoneClean}`}
              ...
              >
                ...
                <div>
                  ...
                  <p className="font-semibold">{CHURCH_INFO.location.fullAddress}</p>
                </div>
              </motion.a>
