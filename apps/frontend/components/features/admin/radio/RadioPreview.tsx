  ...
  return (
    <aside className="overflow-hidden rounded-2xl bg-cecj-green text-white shadow-xl">
      <div className="relative aspect-[16/10] bg-white/5">
        {coverImage ? (
          ...
          <div className="flex h-full items-center justify-center bg-[radial-gradient(circle_at_center,rgba(255,203,50,0.2),transparent_65%)]">
            <RadioIcon className="h-20 w-20 text-cecj-gold" />
          </div>
        )}
      ...
      <div className="space-y-4 p-5">
        <div>
          <p className="text-lg font-bold">{name || "Nom de la radio"}</p>
        ...
        <div className="flex items-center gap-3 rounded-xl bg-white/10 p-3">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-cecj-gold text-cecj-green">
            <svg className="ml-0.5 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M8 5v14l11-7z" />
            </svg>
          </span>
          <div>
            <p className="text-sm font-semibold">Aperçu du lecteur</p>
