...
} from "@/components/features/teachings/player/volume"
...
function VolumeIcon({ volume }: { volume: number }) {
  const level = playerVolumeLevel(volume)
  ...
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      {level === "high" && <path strokeLinecap="round" strokeLinejoin="round" d="M18 7a7 7 0 0 1 0 10" />}
    </svg>
  )
}
...
export function GlobalAudioPlayer() {
  ...
  const [volume, setVolume] = useState(() => {
    ...
    return readStoredPlayerVolume(window.localStorage.getItem(VOLUME_KEY))
  })
  ...
  useEffect(() => {
    ...
    if (audio) {
      ...
      if (!muted && Math.abs(audio.volume - volume) > 0.001) {
        audio.volume = volume
      }
    }
  ...
  const changeVolume = (nextVolume: number) => {
    const normalized = normalizePlayerVolume(nextVolume)
    setVolume(normalized)
    window.localStorage.setItem(VOLUME_KEY, String(normalized))
  }

  const toggleMute = () => {
    changeVolume(volume > 0 ? 0 : lastAudibleVolumeRef.current || DEFAULT_PLAYER_VOLUME)
  }

  const syncVolumeFromAudio = (audio: HTMLAudioElement) => {
    const nextVolume = audio.muted ? 0 : normalizePlayerVolume(audio.volume)
    if (Math.abs(nextVolume - volume) <= 0.001) return
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-cecj-green pb-[env(safe-area-inset-bottom)] text-white shadow-[0_-4px_20px_rgba(0,0,0,0.25)]">
        <audio
          ...
          onVolumeChange={(event) => syncVolumeFromAudio(event.currentTarget)}
          onPlaying={() => {
            setAudioError(false)
            setPlaying(true)
          }}
        ...
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2 sm:flex-nowrap sm:gap-4 sm:py-2.5 lg:px-8">
          ...
          <div className="flex w-full min-w-0 items-center gap-2 sm:w-auto sm:flex-1">
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold">{track?.title ?? station?.nameFr}</p>
            ...
            <div className="flex shrink-0 items-center gap-1 rounded-full bg-white/5 px-1.5">
              ...
              <div className="relative h-9 w-14 sm:w-20">
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-x-[7px] top-1/2 h-1 -translate-y-1/2 overflow-hidden rounded-full bg-white/25"
                >
                  <span
                    className="block h-full rounded-full bg-cecj-gold transition-[width] duration-75"
                    style={{ width: `${playerVolumePercent(volume)}%` }}
                  />
                </div>
                <input
                  type="range"
                  ...
                  aria-valuetext={`${playerVolumePercent(volume)} %`}
                  className="absolute inset-0 h-9 w-full cursor-pointer touch-none appearance-none bg-transparent
                  [&::-webkit-slider-runnable-track]:h-1 [&::-webkit-slider-runnable-track]:bg-transparent
                  [&::-webkit-slider-thumb]:-mt-1.25 [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:w-3.5
                  [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-cecj-gold
                  [&::-webkit-slider-thumb]:shadow-[0_0_0_2px_rgba(2,67,57,0.65)]
                  [&::-moz-range-track]:h-1 [&::-moz-range-track]:bg-transparent
                  [&::-moz-range-progress]:bg-transparent
                  [&::-moz-range-thumb]:h-3.5 [&::-moz-range-thumb]:w-3.5 [&::-moz-range-thumb]:rounded-full
                  [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:bg-cecj-gold
                  [&::-moz-range-thumb]:shadow-[0_0_0_2px_rgba(2,67,57,0.65)]"
                />
              </div>
            </div>
          </div>
